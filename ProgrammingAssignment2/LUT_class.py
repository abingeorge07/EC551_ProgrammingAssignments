...
class FPGA:
    ...
    def show_lut_assignment(self, lut_index):

        if 0 <= lut_index < len(self.luts):

            lut = self.luts[lut_index]

            print(f"LUT {lut_index}: Expression = {lut.logic_expression or 'Not Assigned'}")

        else:

            print("Invalid LUT index")